...
const TYPE_CONFIG: Record<NoticeType, NoticeConfig> = {
  error: { heading: "Something went wrong", message: "An unexpected error occurred.", icon: errorSvg },
  "not-found": { heading: "Page not found", message: "The page you're looking for doesn't exist.", icon: notFoundSvg },
  unauthorized: { heading: "Unauthorized", message: "You need to be logged in to view this page.", icon: unauthorizedSvg },
...
const Notice = ({ type: typeProp, heading: headingProp, message: messageProp, showHomeLink = true }: NoticeProps = {}) => {
  ...
  return (
    <section className="notice" aria-labelledby="notice-heading">
      ...
      <h2 className="notice__heading" id="notice-heading">{headingProp ?? defaultHeading}</h2>
      <p className="notice__message">{messageProp ?? defaultMessage}</p>
      {showHomeLink && <Link className="notice__link" to="/">Go to home page</Link>}
    </section>
  );
};
